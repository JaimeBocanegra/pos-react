...
import { obtenerVentaPorId, obtenerDetallesVenta, cancelarVenta } from "../services/VentaService"
...
export function DetalleVenta() {
    ...
    try {
      ...
      if (Number(venta.Porcentaje) > 0) {
        ...
        if (detalles.length === 1 && detalles[0].PrecioModificado) {
          ...
          descuentoGeneral = (totalSinPrecioModificado * Number(venta.Porcentaje)) / 100
  ...
  const generarPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(12)
      doc.text(`Venta: ${venta.NumeroDocumento || venta.IdVenta || "N/A"}`, 14, 40)
      doc.text(`Fecha: ${venta.FechaRegistro ? formatearFecha(venta.FechaRegistro) : "N/A"}`, 14, 48)
      ...
      doc.text(`Estado: ${venta.Estatus !== "CANCELADO" ? "COMPLETADA" : "CANCELADA"}`, 14, 72)
      doc.setTextColor(0, 0, 0)
      ...
      // Calcular IVA sobre el subtotal con descuento
      const iva = (subtotalConDescuento * Number(venta.Iva || 0)) / 100
      ...
      // Agregar fila de descuento si aplica
      if (mostrarDescuentoGeneral && descuentoGeneral > 0) {
        totalRows.push(["", "", "", "", `Descuento (${venta.Porcentaje}%):`, `- $ ${descuentoGeneral.toFixed(2)}`])
        totalRows.push(["", "", "", "", "Subtotal con descuento:", `$ ${subtotalConDescuento.toFixed(2)}`])
      }

      // Agregar fila de IVA si aplica
      if (Number(venta.Iva) > 0) {
        totalRows.push(["", "", "", "", `IVA (${venta.Iva}%):`, `$ ${iva.toFixed(2)}`])
      }

      // Agregar fila de total
      totalRows.push(["", "", "", "", "Total:", `$ ${Number(venta.MontoTotal).toFixed(2)}`])
      ...
      doc.text(
        `Método de pago: ${
          venta.TPago === "TARJETA" ? "Tarjeta" : venta.TPago === "EFECTIVO" ? "Efectivo" : "Crédito"
        }`,
        14,
        finalTotalsY + 15,
      )
      ...
      if (venta.TPago === "EFECTIVO" && venta.PagoCon) {
        doc.text(`Pagó con: $ ${Number.parseFloat(venta.PagoCon).toFixed(2)} MXN`, 14, finalTotalsY + 25)
        doc.text(`Cambio: $ ${Number.parseFloat(venta.Cambio).toFixed(2)} MXN`, 14, finalTotalsY + 35)
      }

      // Pie de página
      doc.setFontSize(10)
      doc.text(
        `Venta #${venta.IdVenta || venta.NumeroDocumento || ""} - Generado el ${new Date().toLocaleString()}`,
        105,
        280,
        { align: "center" },
      )
  ...
  // Agregar la función para imprimir ticket (formato más compacto para impresoras térmicas)
  const imprimirTicket = async () => {
    ...
    try {
      ...
      // Información de la venta
      doc.setFontSize(9)
      doc.text(`TICKET: ${venta.IdVenta || "N/A"}`, 5, yPos)
      yPos += 4

      doc.text(`FECHA: ${venta.FechaRegistro ? formatearFecha(venta.FechaRegistro) : "N/A"}`, 5, yPos)
      yPos += 4

      doc.text(`CLIENTE: ${venta.NombreCliente || "N/A"}`, 5, yPos)
      yPos += 4

      if (venta.Empleado) {
        doc.text(`EMPLEADO: ${venta.Empleado || "N/A"}`, 5, yPos)
        yPos += 4
      }
      ...
      detalles.forEach((detalle: any) => {
        ...
        const descripcionTruncada = descripcion.length > 20 ? descripcion.substring(0, 18) + "\n"+ descripcion.substring(18, descripcion.length) : descripcion
      ...
      doc.text(`${calcularSubtotal().toFixed(2)}`, 70, yPos, { align: "right" })
      yPos += 4

      // Agregar descuento general si aplica
      if (calcularDescuentoGeneral() > 0) {
        doc.text(`DESC (${venta.Porcentaje}%):`, 30, yPos)
        doc.text(`-${calcularDescuentoGeneral().toFixed(2)}`, 70, yPos, { align: "right" })
        yPos += 4

        doc.text("SUBT C/DESC:", 30, yPos)
        doc.text(`${calcularSubtotalConDescuento().toFixed(2)}`, 70, yPos, { align: "right" })
        yPos += 4
      }

      // Agregar IVA si aplica
      if (Number(venta?.Iva) > 0) {
        doc.text(`IVA (${venta.Iva}%):`, 30, yPos)
        doc.text(`${calcularIVA().toFixed(2)}`, 70, yPos, { align: "right" })
        yPos += 4
      }
      ...
      doc.text(`${calcularTotalFinal().toFixed(2)}`, 70, yPos, { align: "right" })
      yPos += 4
      ...
      if (venta.TPago === "EFECTIVO" && venta.PagoCon) {
        ...
        doc.text(`${Number.parseFloat(venta.PagoCon).toFixed(2)}`, 70, yPos, { align: "right" })
        yPos += 4

        doc.text("CAMBIO:", 30, yPos)
        doc.text(`${Number.parseFloat(venta.Cambio).toFixed(2)}`, 70, yPos, { align: "right" })
        yPos += 4
      }
      ...
      doc.setFontSize(7)
      doc.setFont("helvetica", "normal")
      const fechaX = centrarTexto(`Documento generado: ${new Date().toLocaleString()}`, 7)
      doc.text(`Documento generado: ${new Date().toLocaleString()}`, fechaX, yPos)
  ...
  // Modificar el grupo de botones en el header
  // Buscar:
  /*
  <Group>
    <Button
  ...
  */
  ...
  return (
    <Paper shadow="xs" p="xl" radius="md" w="100%" h="100%" pos="relative" id="comprobante-venta">
      ...
      <Flex direction="column" h="100%" gap="xl">
        ...
        <Flex w="100%" align="center" justify="space-between" className="no-print">
          ...
          <Group>
            ...
            <Button variant="light" color="cyan" leftIcon={<IconTicket size={16} />} onClick={imprimirTicket}>
              Imprimir Ticket
            </Button>
            {venta?.Estatus !== "CANCELADO" && (
              <Button variant="light" color="red" leftIcon={<IconTrash size={16} />} onClick={handleCancelarVenta}>
                Cancelar Venta
              </Button>
            )}
            <IconShoppingBag size={28} color="#228be6" />
          ...
          <>
            ...
            >
              ...
              <Paper p="md" radius="md" withBorder mt="md" sx={{ position: "relative", zIndex: 1 }}>
                ...
                {/* Tabla de totales */}
                <Table striped={false} highlightOnHover={false} style={{ maxWidth: "400px", marginLeft: "auto" }}>
                  <tbody>
                    <tr>
                      <td style={{ textAlign: "right", fontWeight: "bold" }}>Subtotal:</td>
                      <td style={{ textAlign: "right" }}>$ {calcularSubtotal().toFixed(2)} MXN</td>
                    </tr>

                    {calcularDescuentoGeneral() > 0 && (
                      <>
                        <tr>
                          <td style={{ textAlign: "right", fontWeight: "bold" }}>Descuento ({venta.Porcentaje}%):</td>
                          <td style={{ textAlign: "right", color: "red" }}>
                            - $ {calcularDescuentoGeneral().toFixed(2)} MXN
                          </td>
                        </tr>
                        <tr>
                          <td style={{ textAlign: "right", fontWeight: "bold" }}>Subtotal con descuento:</td>
                          <td style={{ textAlign: "right" }}>$ {calcularSubtotalConDescuento().toFixed(2)} MXN</td>
                        </tr>
                      </>
                    )}
                    ...
                    {Number(venta?.Iva) > 0 && (
                      <tr>
                        <td style={{ textAlign: "right", fontWeight: "bold" }}>IVA ({venta.Iva}%):</td>
                        <td style={{ textAlign: "right" }}>$ {calcularIVA().toFixed(2)} MXN</td>
                      </tr>
                    )}
                ...
                {Number(venta?.Porcentaje) > 0 && detalles.some((d) => d.PrecioModificado) && (
              ...
              <Paper p="md" radius="md" withBorder mt="md" sx={{ position: "relative", zIndex: 1 }}>
                ...
                <Text size="sm">
                  ...
                  />
                  No se pueden aplicar descuentos por producto cuando hay descuento general
                </Text>
                <Text size="sm" mt={5}>
                  <IconPercentage
                    size={14}
                  ...
                  />
                  No se pueden aplicar descuentos a productos con precio modificado
                </Text>
                <Text size="sm" mt={5}>
                  <IconPercentage
                    size={14}
                  ...
                  />
                  El descuento general no afecta a productos con precio modificado
                </Text>
              </Paper>
            </Box>
            ...
            <Group position="right" mt="md" className="no-print">
              <Button variant="outline" color="gray" onClick={() => navigate("/ventas")}>
                Volver a Ventas
              </Button>
            </Group>
          </>
